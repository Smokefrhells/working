...
#-Выборка другого оппонента-#
$sel_coliseum_u = $pdo->prepare("SELECT `id`, `statys`, `user_id`, `battle_id`, `user_t_health` FROM `coliseum` WHERE `user_id` != :user_id AND `user_id` != :ank_id AND `statys` = 2 AND `battle_id` = :battle_id AND `user_t_health` > 0");

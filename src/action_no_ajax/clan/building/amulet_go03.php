...
echo clan_level();
echo save();
#-Прокачка амулета клана-#
switch($act){
case 'kash':
if(isset($_GET['clan_id'])){
$clan_id = check($_GET['clan_id']); //ID клана
#-Проверяем что клан существует-#
$sel_clan = $pdo->prepare("SELECT `id`, `gold`, `amulet_lvl`, `level` FROM `clan` WHERE `id` = :clan_id");
...
$gold = (($clan['amulet_lvl'] *1000)+1000);
#-Проверяем что уровень не максимальный-#
if($clan['amulet_lvl'] < 10){
#-Проверяем достаточно ли золота в клане-#
if($clan['gold'] >= $gold){
#-Отнимаем золото в клане-#
$upd_clan = $pdo->prepare("UPDATE `clan` SET `gold` = :gold, `amulet_lvl` = :amulet_lvl WHERE `id` = :clan_id");
$upd_clan->execute(array(':gold' => $clan['gold']-$gold, ':amulet_lvl' => $clan['amulet_lvl']+1, ':clan_id' => $clan['id']));	
#-История клана-#
$ins_clan_l = $pdo->prepare("INSERT INTO `clan_log` SET `type` = :type, `log` = :log, `clan_id` = :clan_id, `time` = :time");
$ins_clan_l ->execute(array(':type' => 7, ':log' => "Уровень Амулета повышен до ".($clan['amulet_lvl']+1)."", ':clan_id' => $clan_id, ':time' => time())); 
...
header("Location: /clan/building/$clan[id]");
$_SESSION['err'] = 'Уровень амулета максимальный!';
exit();	
}
}else{
header("Location: /clan/building/$clan[id]");

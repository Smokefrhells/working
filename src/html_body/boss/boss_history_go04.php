...
$sel_count_l = $pdo->prepare("SELECT COUNT(*) FROM `event_log` WHERE `user_id` = :user_id AND `type` = 3 ORDER BY `time` DESC");
...
$sel_event_b = $pdo->prepare("SELECT * FROM `event_log` WHERE `user_id` = :user_id AND `type` = 3 ORDER BY `time` DESC LIMIT $start, $num");
...
if($_GET['conf'] == 'clear'){
echo'<li><a href="/event_act?act=del_all&type=3&redicet='.$redicet.'"><img src="/style/images/body/basket.png" alt=""/> Подтверждаю</a></li>';
}else{
echo'<li><a href="/boss_history?conf=clear"><img src="/style/images/body/basket.png" alt=""/> Очистить историю</a></li>';
}
echo'</div>';
}
echo'</div>';

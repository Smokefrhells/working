...
#-Навигация-#
$head = '<a href="/p_shop?type=1" style="text-decoration:none;"><span class="body_sel">' . (($_GET['type'] == 9 or $_GET['type'] > 6 or !isset($_GET['type'])) ? "<b>Голова</b>" : "Голова") . '</span></a>';
$body = '<a href="/p_shop?type=2" style="text-decoration:none;"><span class="body_sel">' . ($_GET['type'] == 9 ? "<b>Торс</b>" : "Торс") . '</span></a>';
$gloves = '<a href="/p_shop?type=3" style="text-decoration:none;"><span class="body_sel">' . ($_GET['type'] == 9 ? "<b>Руки</b>" : "Руки") . '</span></a>';
$shield = '<a href="/p_shop?type=4" style="text-decoration:none;"><span class="body_sel">' . ($_GET['type'] == 9 ? "<b>Защита</b>" : "Защита") . '</span></a>';
$arm = '<a href="/p_shop?type=5" style="text-decoration:none;"><span class="body_sel">' . ($_GET['type'] == 9 ? "<b>Оружие</b>" : "Оружие") . '</span></a>';
$legs = '<a href="/p_shop?type=6" style="text-decoration:none;"><span class="body_sel">' . ($_GET['type'] == 9 ? "<b>Ноги</b>" : "Ноги") . '</span></a>';
...
#-Тип снаряжения-#
if ($_GET['type'] > 9 or !isset($_GET['type'])) {
    $type = 1;
} else {
...
$sel_weapon = $pdo->prepare("SELECT * FROM `weapon` WHERE `type` = :type AND (`no_magaz` = 0 OR `no_magaz` = 15) ORDER BY `level`");
...
#-Вывод снаряжения-#
        ...
        echo '<span style="color: ' . color_w($weapon['level']) . ';"><img src="' . star($weapon['level']) . '" alt=""/><b>' . $weapon['name'] . '</b> ' . ($user['level'] >= $weapon['level'] ? "<span class='green'>[$weapon[level] ур.]</span>" : "<span class='red'>[$weapon[level] ур.]</span>") . ' ' . ($weapon['no_magaz'] == 15 ? "<span class='green'>[Праздничный]</span>" : "") . '</span><br/>';
        echo '<div class="weapon_param"><img src="/style/images/user/sila.png" alt=""/>' . $weapon['sila'] . ' <img src="/style/images/user/zashita.png" alt=""/>' . $weapon['zashita'] . ' <img src="/style/images/user/health.png" alt=""/>' . $weapon['health'] . ' ' . $w_param . '<br/>' . ($weapon['no_magaz'] == 15 ? "Праздничная валюта" : "<img src='/style/images/many/gold.png' alt=''/><span style='color: $color_g;'>$weapon[gold]</span> <img src='/style/images/many/silver.png' alt=''/><span style='color: $color_s;'>$weapon[silver]</span>") . '<br/>';
        echo '</div></div>';
